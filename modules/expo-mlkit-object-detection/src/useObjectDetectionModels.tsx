...
import { ExpoMLKitCustomObjectDetector } from "./ExpoMLKitCustomObjectDetector";
import { ExpoMLKitDefaultObjectDetector } from "./ExpoMLKitDefaultObjectDetector";
import {
  ExpoMLKitObjectDetectionContext,
  ModelAssets,
  useExpoMLKitObjectDetectionContext,
...
type Models<T extends Record<string, any>> = {
  [K in keyof T | "default"]: ExpoMLKitObjectDetector;
};
...
export function useObjectDetectionModels<T extends AssetRecord>({
  ...
  defaultModelOptions?: ExpoMLKitObjectDetectorOptions;
}) {
  ...
  useEffect(() => {
    async function loadModel(
      name: string,
      model: ExpoMLKitObjectDetector
    ): Promise<[string, ExpoMLKitObjectDetector]> {
      await model.load();
      return [name, model];
    }
    ...
    async function loadModels() {
      const modelPromises: Promise<
        [string, ExpoMLKitObjectDetector | undefined]
      >[] = Object.entries(assets).map(
        async ([name, modelInfo]): Promise<
          [string, undefined | ExpoMLKitObjectDetector]
        > => {
          ...
          const model = new ExpoMLKitCustomObjectDetector(
            name,
      ...
      if (loadDefaultModel) {
        const model = new ExpoMLKitDefaultObjectDetector(defaultModelOptions);
...
function ObjectDetectionModelProviderComponent<T extends ModelAssets>({
  ...
  return (
    <ExpoMLKitObjectDetectionContext.Provider value={{ models }}>
      {children}
    </ExpoMLKitObjectDetectionContext.Provider>
  );
}
