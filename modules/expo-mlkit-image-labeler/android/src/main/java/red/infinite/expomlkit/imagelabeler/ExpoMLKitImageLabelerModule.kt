...
class ExpoMLKitImageLabelerModule : Module() {
    ...
    override fun definition() = ModuleDefinition {
        ...
        AsyncFunction("addModel") { spec: ExpoMLKitImageLabelerSpec, promise: Promise ->
            try {
                ...
                promise.reject(
                    CodedException("ExpoMLKitImageLabelerModule - Classifier Error: ${e.message}", e)
                )
            }
        }
        ...
        AsyncFunction("classifyImage") { modelName: String, imagePath: String, promise: Promise ->
            ...
            runBlocking suspend@{
                ...
                if (model == null) {
                    promise.reject(
                        CodedException("rExpoMLKitImageLabelerModule - Classifier Error: Model $modelName not found")
                    )
                    ...
                    val result = model.classifyImage(imagePath).getOrElse {
                        promise.reject(
                            CodedException(
                                "ExpoMLKitImageLabelerModule - Classifier Error: ${it.message}", it
                            )
                        )
                    }
        ...
        AsyncFunction("updateOptionsAndReload") { modelName:String, options: ExpoMLKitImageLabelerOptions, promise:Promise ->
            ...
            runBlocking {
                try {
                    ...
                    promise.reject(
                        CodedException("ExpoMLKitImageLabelerModule - Error Updating Options: ${e.message}", e)
                    )
                }
            }
